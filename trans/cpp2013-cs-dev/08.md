...
C#和 C++都是面向对象的语言，这意味着这两种语言都支持创建对象，这些对象不仅封装数据，还封装数据上的相关方法或操作。这两种语言还提供了将不同对象相互关联的机制，范围从运算符重载到继承、接口和参数多态性。这两种语言都提供了限制和控制数据访问的机制，从限制可访问性和可见性到使用属性扫描和过滤数据。

在这一章中，我们将讨论其中的一些机制，因为它们与 C#的机制不同。

## 多态性

多态性，来自希腊语 poly morphos，意思是“许多形状”，是许多科学领域的常用术语，并正在迅速进入日常用语。在面向对象的编程中，多态性指的是对象可以被分组和分类的方式，因此一个对象可以被视为不同的对象、一组对象的成员，或者根据定义的特征被访问。下面的列表强调了多态性中的一些重要概念:
...
*   关键字`virtual`的使用与 C#中相同，它允许派生类实现或重写方法。在多态性术语中，最顶层(基)方法是实现，派生方法是重写。
...
![A978-1-4302-6707-2_8_Fig1_HTML.jpg](A978-1-4302-6707-2_8_Fig1_HTML.jpg)
