...
让我们转向 Visual Studio 对象浏览器。展开`[mscorlib] System.Threading`下的`Thread`类，找到`Thread`类。我们对这个构造函数重载感兴趣。
...
我们首先创建一个名为`Locker`的类，它是`System::Threading:: Monitor`类的容器。当进入锁定节时调用构造函数，当退出锁定节时调用析构函数。
