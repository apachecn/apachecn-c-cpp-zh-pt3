...
![image](images/sq.jpg) **注意**这就是所谓的*“独生子女”模式*！
...
![image](images/sq.jpg) **注意**没有定义是必要的，因为它从来没有被调用过。
...
![image](images/sq.jpg) **评论**当然，在包含头文件的时候，`init`的一个文件静态实例也被放在这个文件中。
...
![image](images/sq.jpg) **注意**这很大程度上取决于这样一个事实，即在任何动态初始化发生之前，静态存储区被设置为零。
