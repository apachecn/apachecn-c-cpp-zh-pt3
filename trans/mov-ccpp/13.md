...
![image](images/sq.jpg) **注**你可以用`calloc()`或者`realloc()`代替`malloc()`。
...
![image](images/sq.jpg) **注意**然而，C++为内存泄漏提供了大量的其他机会。
...
![image](images/sq.jpg) **注意**您也可能开始看到过载的`new`和`delete`对于查找内存泄漏的潜力。
...
![image](images/sq.jpg) **注**这台电脑用四个字节表示一个`int`。
