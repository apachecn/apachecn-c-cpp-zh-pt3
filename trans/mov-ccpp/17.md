...
![image](images/sq.jpg) **注意**你会在代码的一个适当的部分结束——称为异常处理程序——它可能远离抛出异常的地方。
...
![image](images/sq.jpg) **注**详见本章后面的“资源管理”一节。
...
清单 17-7 展示了没有完成的构造函数不会调用相关的析构函数。它还显示了在创建对象数组的过程中抛出异常时会发生什么。
...
当编写带有异常的代码时，特别重要的是，您总是问:“如果发生异常，我的资源会被适当地清理吗？”大多数时候你是相当安全的，但是在构造函数中有一个特殊的问题:如果一个异常在构造函数完成之前被抛出，相关的析构函数将不会被调用。因此，在编写构造函数时，你必须特别勤奋。
...
![image](images/sq.jpg) **注**本章稍后将讨论该指南的变体。
...
![image](images/sq.jpg) **注意**这种行为是`unexpected()`独有的。
...
![image](images/sq.jpg) **注**回想一下前面关于异常安全以及为什么标准 C++库中没有异常规范的讨论。
...
![image](images/sq.jpg) **注**参见本节后面对“零成本模式”的描述。
