...
再次参考清单 4-1 中的[——你会看到，在动作被添加到菜单和工具栏后，构造器中的最后一个调用创建了一个状态栏，并在其中显示了消息`"Done"`。`statusBar()`方法的工作方式就像`menuBar()`一样:在第一次调用时创建并返回一个条，然后在随后的调用中返回一个指向它的指针。](#constructor_of_the_sdi_main_window)
