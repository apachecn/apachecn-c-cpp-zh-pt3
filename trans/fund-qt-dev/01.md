...
在[清单 1-20](#showing_all_key-value_pairs_on_the_debug) 中，`[]`操作符用于访问列表中已知的条目。如果使用`[]`操作符来获取一个不存在的项目(如下所示)，则会创建一个新项目。新项等于零或使用默认构造函数创建。
