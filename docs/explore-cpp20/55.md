...
列表中可能出现的另一种需求是类型需求，它只是类型的名称，比如成员类型名称或模板专门化。如果类型有效，则要求为真。例如，所有标准容器都有一个名为`size_type`的成员类型。如果你想写一个`size()`函数来检查一个`size_type`成员和一个`size()`成员函数，你可以如清单 [55-4](#PC8) 所示来写。
...
如果`R`是一个大小已知的范围，并且该大小可以在常数时间内确定(不是通过迭代该范围)，则产生一个真约束。
