...
例如，假设您希望能够使用标准(或多或少)书目格式打印图书馆中的任何类型的作品(参见清单 [38-1](38.html#PC1) )。对于书籍，我使用的格式是
...
其他新的语言特性以新的方式使用现有的关键字，例如用于构造函数的`=default`和`=delete`。但是增加了一些新的关键字，它们带来了破坏现有代码的风险。因此，委员会试图选择不太可能与现有用户选择的名字冲突的名字。在本书的后面部分，你会看到一些新关键词的例子，以及特殊语境中特殊词汇的其他新颖用法，避免将这些特殊词汇作为关键词。
