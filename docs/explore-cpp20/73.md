...
一个`constexpr`或`consteval`函数有一些限制。大多数限制适用于这两种函数。函数中的返回类型、参数类型和变量类型必须是所谓的*文字*类型。文字类型是可以在编译时构造和销毁的类型。内置的基本类型、枚举和指针显然适合这个类。对于一个文本类，它必须有一个`constexpr`析构函数和文本数据成员。被调用的构造函数必须是`constexpr`，尽管这个类可以有其他不是`constexpr`的构造函数。

一个`constexpr`构造函数只能调用其他`constexpr`构造函数，这意味着所有基类构造函数都必须是`constexpr`。如果一个类有一个`constexpr`析构函数，那么它的所有基类都必须有`constexpr`析构函数(或者根本没有析构函数使用`=delete`)。
...
`constexpr`说明符也适用于命名对象。一个`constexpr`对象是隐式的`const`。它必须有一个只调用`constexpr`函数或`constexpr`构造函数的初始化器。
