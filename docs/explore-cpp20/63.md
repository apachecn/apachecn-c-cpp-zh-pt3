...
构造一个`std::regex`对象来编译正则表达式。如果正则表达式包含错误，构造函数抛出`std::regex_error`。记住反斜杠(`\`)在字符串中有特殊的含义，在正则表达式中用作元字符，所以对于`\\S`必须重复使用。在`\t`中留下一个反斜杠，因为它是一个普通的制表符。`regex_match()`函数根据正则表达式测试一个字符串，并且根据您传递的参数，也可以返回捕获组。复制清单 [62-5](62.html#PC5) 并将解析器更改为使用正则表达式会产生清单 [63-1](#PC4) 。
