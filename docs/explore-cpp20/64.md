...
要创建一个新的`variables`对象，调用`std::make_unique<variables>()`。当唯一指针的所有者被销毁时，那么`variables`对象也将被销毁。修改`artifact`类，将`std::unique_ptr`用于`variables`映射。将您的解决方案与清单 [64-5](#PC9) 进行比较。
