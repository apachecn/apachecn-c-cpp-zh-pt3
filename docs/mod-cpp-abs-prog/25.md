...
以下示例引入了名为`protected:`的新访问说明符。派生类本身可以访问基类的`protected`成员。`protected`访问说明符允许访问基类和派生类，但不允许访问对象:
...
`MyBaseClass`类有一个名为`dowork()`的函数，带有一个`virtual`说明符。虚拟意味着该函数可以在后续的派生类中被重写/重定义，并且适当的版本将通过多态对象被调用。派生类中有一个同名的函数和相同类型的参数(在我们的例子中没有)。
