...
那么，组合模式是什么呢？本质上，我们试图给单个对象和对象组一个相同的接口。当然，定义一个接口并在两个对象中实现它是很容易的。但是同样地，您可以在适当的时候尝试利用 duck 类型化机制，比如`begin()/end()`。 [<sup>1</sup>](#Fn1) Duck typing，总的来说是一个可怕的想法，因为它依赖于秘密知识，而不是在某个接口中明确定义。顺便说一下，没有什么可以阻止你用`begin()`和`end()`创建一个显式接口，但是迭代器类型是什么呢？
